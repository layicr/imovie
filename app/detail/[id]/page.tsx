...
export default function Detail() {
  ...
  const [status, setStatus] = useState<Status>("plan");
  ...
  async function load() {
    try {
      ...
      if (res.ok) {
        ...
        setData(d);
        setStatus(d.record?.status || "plan");
        setRating(d.record?.rating || 8);
        setTags(d.record?.tags || "");
      } else {
  ...
  if (loading) return <div className="py-20 text-center text-subtle">{t("detail.loading")}</div>;
  if (!data) return <div className="py-20 text-center text-subtle">{error || t("detail.notFoundGeneric")}</div>;
